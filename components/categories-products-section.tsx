        ...
        <div className="mb-2 hidden rounded-xl border border-slate-200 bg-white px-2.5 py-2.5 shadow-[0_8px_18px_rgba(15,42,77,0.08)] md:mb-4 md:block md:rounded-2xl md:px-5 md:py-4">
          <div className="flex flex-wrap items-center justify-between gap-2">
            ...
            <div className="flex flex-wrap gap-2">
              <Link
                href="/offres"
                className="inline-flex rounded-full border border-orange-200 bg-orange-50 px-2 py-0.5 text-[9px] font-bold text-brand-orange transition hover:border-brand-orange sm:px-3 sm:py-1 sm:text-xs"
              >
                Offres actives
              </Link>
              <Link
                href="/produits"
                className="inline-flex rounded-full border border-slate-300 bg-white px-2 py-0.5 text-[9px] font-bold text-brand-blue transition hover:border-brand-orange hover:text-brand-orange sm:px-3 sm:py-1 sm:text-xs"
              >
                Catalogue complet
              </Link>
            </div>
          </div>
          <p className="mt-1 text-[11px] text-slate-600 sm:mt-2 sm:text-sm">
            Top ventes recommandees + categories les plus demandees pour commander plus vite.
          </p>
        </div>

        <div className="grid gap-2.5 sm:gap-5 lg:grid-cols-12">
          <aside className="min-w-0 lg:col-span-3">
            <div className="w-full max-w-full overflow-hidden lg:hidden">
              <div className="mb-1.5 flex items-center justify-between gap-2">
                <h3 className="text-sm font-extrabold uppercase tracking-wide text-brand-blue">
                  Categories phares
                </h3>
                <Link
                  href="/produits"
          ...
          <div className="min-w-0 lg:col-span-9">
            <div className="mb-1.5 flex items-center justify-between gap-2 sm:mb-3">
              ...
              <Link
                href="/produits"
                className="inline-flex items-center gap-1 text-xs font-bold text-brand-blue transition hover:text-brand-orange sm:text-xl"
              >
                Voir tout
                <span aria-hidden>&rarr;</span>
              </Link>
            </div>
            <div className="hidden grid-cols-2 gap-3 sm:grid sm:grid-cols-2 sm:gap-4 xl:grid-cols-4">
