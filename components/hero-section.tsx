...
export const HeroSection = () => {
  ...
  return (
    <section className="bg-[#f1f3f5] pb-5 pt-1.5 sm:pb-7 sm:pt-3">
      <div className="mx-auto max-w-7xl px-4 sm:px-5 lg:px-6">
        <div className="relative overflow-hidden rounded-2xl shadow-[0_14px_28px_rgba(15,42,77,0.16)] sm:rounded-[28px] sm:shadow-[0_16px_34px_rgba(15,42,77,0.18)]">
          <Image
            src={siteImages.hero}
            alt="Magasin de materiaux de construction a Fes"
            width={1400}
            height={720}
            className="h-[330px] w-full object-cover object-center sm:h-[520px]"
            priority
          />
          <div className="absolute inset-0 bg-gradient-to-r from-brand-blue/90 via-brand-blue/78 to-brand-blue/35" />
          <div className="absolute inset-0 bg-gradient-to-t from-black/35 via-transparent to-transparent" />

          <div className="absolute inset-0 flex items-center px-4 py-4 sm:px-8 sm:py-8 lg:px-12">
            <div className="max-w-[700px]">
              <p className="inline-flex rounded-full border border-white/45 bg-white/10 px-2.5 py-0.5 text-[10px] font-bold uppercase tracking-[0.08em] text-slate-100 backdrop-blur sm:px-3 sm:py-1 sm:text-xs">
                {homepageContent.hero.badge}
              </p>
              <h1 className="mt-2 text-[1.55rem] font-extrabold uppercase leading-[1.04] tracking-tight text-white drop-shadow-[0_4px_12px_rgba(15,42,77,0.45)] sm:mt-3 sm:text-5xl sm:leading-[1.02] lg:text-[3.5rem]">
                {homepageContent.hero.title}
              </h1>
              <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-100 sm:mt-4 sm:text-[1.08rem] sm:leading-relaxed">
              ...
              <div className="mt-3 flex flex-wrap gap-1.5 sm:mt-4 sm:gap-2">
                {salesHighlights.map((item) => (
                  <span
                    key={item}
                    className="inline-flex rounded-full bg-white/95 px-2.5 py-0.5 text-[10px] font-bold text-brand-blue sm:px-3 sm:py-1 sm:text-xs"
                  >
                    {item}
                  </span>
                ))}
              ...
              <div className="mt-4 flex flex-col gap-2 sm:mt-7 sm:flex-row sm:flex-wrap sm:gap-3">
                <Link
                  href="/produits"
                  className="inline-flex h-10 items-center justify-center rounded-full bg-brand-orange px-5 text-sm font-extrabold text-white shadow-[0_10px_18px_rgba(249,115,22,0.3)] transition hover:bg-brand-orangeDark sm:h-12 sm:px-7 sm:text-base sm:shadow-[0_12px_22px_rgba(249,115,22,0.34)]"
                >
                  Commander maintenant
                </Link>
                <Link
                  href="/offres"
                  className="inline-flex h-10 items-center justify-center rounded-full border border-white/55 bg-white/10 px-4 text-xs font-bold text-white backdrop-blur transition hover:bg-white/20 sm:h-12 sm:px-6 sm:text-sm"
                >
                ...
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="inline-flex h-10 items-center justify-center gap-2 rounded-full bg-[#1db954] px-4 text-xs font-bold text-white shadow-[0_8px_14px_rgba(34,197,94,0.24)] transition hover:brightness-95 sm:h-12 sm:px-5 sm:text-sm sm:shadow-[0_10px_18px_rgba(34,197,94,0.28)]"
                >
        ...
        <aside className="mt-2 rounded-xl border border-slate-200 bg-white p-3 shadow-card lg:hidden">
          <h2 className="text-base font-bold text-brand-blue">{homepageContent.deliveryCard.title}</h2>
          <p className="mt-1.5 text-xs font-semibold text-slate-700">{freeDeliveryText}</p>
          <p className="mt-1 text-xs text-slate-600">{paidDeliveryText}</p>
          <div className="mt-2 grid grid-cols-2 gap-1.5">
            <p className="rounded-lg border border-slate-200 bg-slate-50 px-2 py-1.5 text-[10px] font-semibold text-slate-700">
              Paiement a la livraison
            </p>
            <p className="rounded-lg border border-slate-200 bg-slate-50 px-2 py-1.5 text-[10px] font-semibold text-slate-700">
