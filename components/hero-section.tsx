...
export const HeroSection = () => {
  ...
  return (
    <section className="bg-[#f1f3f5] pb-3 pt-2 sm:pb-7 sm:pt-3">
      <div className="mx-auto max-w-7xl px-3 sm:px-5 lg:px-6">
        <div className="relative overflow-hidden rounded-xl shadow-[0_10px_22px_rgba(15,42,77,0.15)] sm:rounded-[28px] sm:shadow-[0_16px_34px_rgba(15,42,77,0.18)]">
          ...
          <div className="absolute inset-0 flex items-center px-3 py-2 sm:px-8 sm:py-8 lg:px-12">
            <div className="max-w-[700px]">
              <p className="inline-flex rounded-full border border-white/45 bg-white/10 px-2.5 py-1 text-xs font-bold uppercase tracking-[0.08em] text-slate-100 backdrop-blur sm:px-3 sm:py-1 sm:text-sm">
                {homepageContent.hero.badge}
              </p>
              <h1 className="mt-2 text-lg font-extrabold uppercase leading-[1.08] tracking-tight text-white drop-shadow-[0_4px_12px_rgba(15,42,77,0.45)] sm:mt-3 sm:text-5xl sm:leading-[1.02] lg:text-[3.5rem]">
                {homepageContent.hero.title}
              </h1>
              <p className="mt-1.5 max-w-xl text-sm leading-5 text-slate-100 md:hidden">
                {mobileSubtitle}
              </p>
              <p className="mt-1.5 hidden max-w-xl text-sm leading-5 text-slate-100 md:mt-4 md:block md:text-[1.08rem] md:leading-relaxed">
                {homepageContent.hero.subtitle}
              </p>

              <div className="mt-2 hidden max-w-full gap-2 overflow-x-auto pb-0.5 [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden md:mt-4 md:flex md:flex-wrap md:overflow-visible md:pb-0 md:gap-2">
                {salesHighlights.map((item) => (
                  <span
                    key={item}
                    className="inline-flex shrink-0 rounded-full bg-white/95 px-2.5 py-1 text-xs font-bold text-brand-blue sm:px-3 sm:py-1 sm:text-sm"
                  >
                    {item}
                  </span>
                ))}
              ...
              <div className="mt-3 flex flex-wrap gap-2 sm:mt-7 sm:flex-row sm:flex-wrap sm:gap-3">
                <Link
                  href="/produits"
                  className="inline-flex h-11 items-center justify-center rounded-full bg-brand-orange px-4 py-3 text-base font-extrabold text-white shadow-[0_8px_14px_rgba(249,115,22,0.3)] transition hover:bg-brand-orangeDark sm:h-12 sm:px-7 sm:text-base sm:shadow-[0_12px_22px_rgba(249,115,22,0.34)]"
                >
                  Commander maintenant
                </Link>
                <Link
                  href="/offres"
                  className="hidden h-11 items-center justify-center rounded-full border border-white/55 bg-white/10 px-4 py-3 text-base font-bold text-white backdrop-blur transition hover:bg-white/20 md:inline-flex md:h-12 md:px-6 md:text-base"
                >
                ...
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="hidden h-11 items-center justify-center gap-1.5 rounded-full bg-[#1db954] px-4 py-3 text-base font-bold text-white shadow-[0_8px_14px_rgba(34,197,94,0.24)] transition hover:brightness-95 md:inline-flex md:h-12 md:px-5 md:text-base md:shadow-[0_10px_18px_rgba(34,197,94,0.28)]"
                >
        ...
        <aside className="mt-2 hidden rounded-xl border border-slate-200 bg-white p-3 shadow-card md:block lg:hidden">
          <h2 className="text-sm font-bold text-brand-blue">{homepageContent.deliveryCard.title}</h2>
          <p className="mt-1.5 text-sm font-semibold text-slate-700">{freeDeliveryText}</p>
          <p className="mt-1 text-sm text-slate-600">{paidDeliveryText}</p>
          <div className="mt-2 grid grid-cols-2 gap-2">
            <p className="rounded-lg border border-slate-200 bg-slate-50 px-2.5 py-2 text-sm font-semibold text-slate-700">
              Paiement a la livraison
            </p>
            <p className="rounded-lg border border-slate-200 bg-slate-50 px-2.5 py-2 text-sm font-semibold text-slate-700">
